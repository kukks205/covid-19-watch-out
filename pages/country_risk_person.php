...
<script>
    ...
    angular.module('ngApp', ['angularMoment', 'datatables', 'datatables.fixedheader', 'tmh.dynamicLocale',
        ...
        .controller('homeCtrl', function ($scope, $http, moment, DTOptionsBuilder, DTColumnDefBuilder) {
            ...
            $http.post("./get_country_risk_person.php")
                .success((res) => {
                    var data = res;
                    $scope.data = res;
    ...
    <div style="padding: 20px;height:100%">
        ...
        <div class="clearfix">
            ...
            <h2 class="float-left">
                ...
                <b>ผู้เดินทางมาจากประเทศเสี่ยง</b>
            </h2>
        </div>
        <div class="line"></div>
            ...
            <table datatable="ng" dt-options="showCase.dtOptions" dt-column-defs="showCase.dtColumnDefs"
                ...
                <thead>
                    <tr>
                        ...
                        <th scope="col" width='15px'>CID</th>
                ...
                <tbody>
                    <tr ng-repeat="m in data">
                        ...
                        <td>{{m.tambonname}}</td>
                        <td>{{m.telphone}}</td>
                        <td>{{m.from_country_name}}</td>
                        <td>{{m.from_city}}</td>
                        <td>{{thaiDate(m.date_arrival_home)}}</td>
                        <td>{{thaiDate(m.date_quarantine)}}</td>
                        <td><a href="index.php?url=pages/register_detail.php&cid={{m.cid}}"
                                class="btn btn-sm btn-success "><i class="fas fa-user-check"></i></a></td>
                    </tr>
                </tbody>
            </table>
        </div>
